...
cron.schedule("10 0 * * *", function() {
    ...
    connection.query('select device_id, date(test_date) AS win_date from (select *, sum(`test_point`) as total from yolnisanlari_contest_info where date(test_date) = subdate(current_date, 1) group by device_id having sum(`test_point`) >= 1000 AND sum(`watched_video`) > 0) t order by rand() limit 1', function(err, result) {
        if(err) throw err
